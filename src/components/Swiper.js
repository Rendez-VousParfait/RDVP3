...
import { FaHeart, FaTimes, FaUtensils, FaMoneyBillWave, FaBed, FaWalking } from 'react-icons/fa';
...
const Swiper = ({ formData, handleInputChange, nextStep }) => {
  ...
  const handleSwipe = useCallback(async (isLike) => {
    ...
    setTimeout(async () => {
      ...
      if (isLike) {
        newLikedTypes[currentOffer.type] = (newLikedTypes[currentOffer.type] || 0) + 1;
      } else {
        newDislikedTypes[currentOffer.type] = (newDislikedTypes[currentOffer.type] || 0) + 1;
      }
      ...
      if (swipeCount + 1 >= 5) {
        handleInputChange('swiperPreferences', { liked: newLikedTypes, disliked: newDislikedTypes });
        nextStep();
      }
    }, 300);
  }, [currentIndex, offers, swipeCount, likedTypes, dislikedTypes, handleInputChange, nextStep]);
    ...
    <div className={styles['search-step']}>
      ...
      <div className={styles['swiper-container']} {...handlers}>
        <div className={`${styles['offer-card']} ${styles[direction]}`}>
          <img src={currentOffer.image} alt={currentOffer.name} className={styles['offer-image']} />
          <div className={styles['offer-info']}>
            <h3 className={styles['offer-name']}>{currentOffer.name}</h3>
            <p className={styles['offer-description']}>{currentOffer.description}</p>
      ...
      <div className={styles['swiper-buttons']}>
        <button className={styles['dislike-button']} onClick={() => handleSwipe(false)}>
          <FaTimes />
        </button>
        <button className={styles['like-button']} onClick={() => handleSwipe(true)}>
          <FaHeart />
        </button>
      </div>
      ...
      <p className={styles['swipe-count']}>Swipes restants : {5 - swipeCount}</p>
    </div>
  );
};
