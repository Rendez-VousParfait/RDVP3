...
const PersonalizeAdvanced = ({ formData, handleInputChange, nextStep, prevStep }) => {
  const [personalizationType, setPersonalizationType] = useState(formData.personalizationType || '');
  ...
  const activities = ['Concerts', 'Randonnées', 'Cinéma', 'Sport', 'Loisirs créatifs'];
  const ambiances = ['Romantique', 'Aventure', 'Détente', 'Culturelle', 'Festive'];
