        ...
        {activityTypes.map((type) => (
          <label key={type}>
            <input
              ...
              onChange={() => {
                const updatedTypes =
                  formData.activityPreferences.types?.includes(type)
                    ? formData.activityPreferences.types.filter(
                        (t) => t !== type,
                      )
      ...
      <div className="special-interests">
        ...
        {specialInterests.map((interest) => (
          <label key={interest}>
            <input
              type="checkbox"
              checked={formData.activityPreferences.specialInterests?.includes(
                interest,
              )}
              onChange={() => {
                const updatedInterests =
                  formData.activityPreferences.specialInterests?.includes(
                    interest,
                  )
                    ? formData.activityPreferences.specialInterests.filter(
                        (i) => i !== interest,
                      )
                    : [
                        ...(formData.activityPreferences.specialInterests ||
                          []),
                        interest,
                      ];
                handleInputChange("activityPreferences", {
                  ...formData.activityPreferences,
                  specialInterests: updatedInterests,
                });
              }}
            />
            {interest}
